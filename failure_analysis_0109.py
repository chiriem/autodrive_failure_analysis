...
high_ratio_th = st.slider("마스크 비율 상한 (매우 높음)", min_value=0.0, max_value=1.0, value=0.25, step=0.005)

err_pctl = None
proc_pctl = None
if ABS_ERROR_COL in df.columns:
    err_pctl = st.slider("오차 상위 분위(%)", min_value=80, max_value=99, value=95, step=1)
if PROC_COL in df.columns:
    proc_pctl = st.slider("처리시간 상위 분위(%)", min_value=80, max_value=99, value=95, step=1)

include_missing_error = st.checkbox("오차 기록 누락도 후보로 포함", value=True)
sample_n_part4 = st.slider("산점도 표본 수(너무 크면 느려짐)", min_value=200, max_value=10000, value=3000, step=200)

d = df.copy()

# normalize types (safe)
...
if ERROR_COL in d.columns:
    err_missing = d[ERROR_COL].isna()
if ABS_ERROR_COL in d.columns and err_pctl is not None:
    err_th_value = float(pd.to_numeric(d[ABS_ERROR_COL], errors="coerce").quantile(err_pctl / 100.0))
    err_high = pd.to_numeric(d[ABS_ERROR_COL], errors="coerce").ge(err_th_value)

proc_high = pd.Series(False, index=d.index)
proc_th_value = None
if PROC_COL in d.columns and proc_pctl is not None:
    proc_th_value = float(pd.to_numeric(d[PROC_COL], errors="coerce").quantile(proc_pctl / 100.0))
    proc_high = pd.to_numeric(d[PROC_COL], errors="coerce").ge(proc_th_value)
...
if cand.empty:
    ...
    # scatter
    axis_options = [MASK_RATIO_COL]
    if ABS_ERROR_COL in df.columns:
        axis_options.append(ABS_ERROR_COL)
    if PROC_COL in df.columns:
        axis_options.append(PROC_COL)

    x_col4 = st.selectbox("산점도 X축", options=axis_options, index=0)
    y_col4 = st.selectbox("산점도 Y축", options=axis_options, index=min(1, len(axis_options)-1))

    plot_df = cand.dropna(subset=[x_col4, y_col4]).copy()
    ...
    st.subheader("후보 프레임 목록")
    show_cols = [EVENT_ID_COL, RUN_ID_COL, TS_COL, WEATHER_COL, TOD_COL, MASK_RATIO_COL]
    if ABS_ERROR_COL in cand.columns:
        show_cols.append(ABS_ERROR_COL)
    if PROC_COL in cand.columns:
        show_cols.append(PROC_COL)
    if ERROR_COL in cand.columns:
        show_cols.append(ERROR_COL)
    show_cols += ["Primary Tag", "Candidate Tags"]

    show_cols = [c for c in show_cols if c in cand.columns]
    st.dataframe(
        cand[show_cols].sort_values(["Primary Tag", MASK_RATIO_COL], ascending=[True, True]).head(200),
        column_config=_column_config_for(show_cols),
        use_container_width=True,
        height=420,
    )

...
if FAIL_FLAG_COL not in df.columns:
    ...
    if _n_fail == 0:
        ...
        with c4:
            if mode_top is None:
                st.metric(f"최다 {MODE_COL}", "N/A")
            else:
                st.metric(f"최다 {MODE_COL}", mode_top[0], delta=f"n={mode_top[1]:,}")

        # --- 2행 카드
        c5, c6, c7, c8 = st.columns(4)
        with c5:
            st.metric(f"{ABS_ERROR_COL} 중앙값", "N/A" if abs_med is None else f"{abs_med:.2f}")
        with c6:
            st.metric(f"{ABS_ERROR_COL} p95", "N/A" if abs_p95 is None else f"{abs_p95:.2f}")
        with c7:
        ...
        if show_freq_tables:
            ...
            if mode_top is not None:
                with colB:
                    st.markdown(f"### {MODE_COL} 상위")
                    vc = _fail_df[MODE_COL].astype(str).fillna("nan").value_counts().head(8)
                    t = pd.DataFrame({
                        MODE_COL: vc.index,
                        "count": vc.values,
                        "rate(%)": (vc.values / _n_fail * 100.0).round(2),
